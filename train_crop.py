...
class_list = ['plane', 'small-vehicle', 'ship']
    ...
    if len(boxes_all) > 0:
        ...
        for start_h in range(0, shape[0], 256):
            for start_w in range(0, shape[1], 256):
                ...
                if len(idx) > 0:
                    xml = os.path.join(save_dir, 'Annotation', "%s_%04d_%04d.xml" % (file_idx, top_left_row, top_left_col))
...
raw_data = 'VOCdevkit_train/raw/'
raw_images_dir = os.path.join(raw_data, 'imagesthird220sub')
raw_label_dir = os.path.join(raw_data, 'labelTxtpcsthird220sub')

save_dir = 'VOCdevkit_train/xmlclippedsub'
