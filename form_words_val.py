    ...
    for xshift in range(0, 800, step):
        for yshift in range(0, 800, step):
            if(len(lessthan) > 0):
                ...
                x1 = int(step/2) -1 - int(width/2)   + xshift
                y1 = int(step/2) -1 - int(height/2)  + yshift 
                x2 = x1 + width   
                y2 = y1 + height  
                #import pdb; pdb.Pdb(skip=['django.*']).set_trace()
                A[y1:y2 ,x1:x2,:] = img
...
imageprintpath      = r"E:\1MyWork\1mypapers\paper6\Object\darknet\data\alpha\bletters\exper3\val\alltrainletters/" 
contextpath         = r"E:\1MyWork\1mypapers\paper6\Object\darknet\data\alpha\bletters\exper3\val\alltraincontext100/"

outputlinuxpath     = r"E:\1MyWork\1mypapers\paper6\Object\darknet\data\alpha\cwords\exper4/"

linuxpathsource     = r"C:\Users\nour.laban\source\repos\darknet\out\build\x64-Debug\exper3\words\store/"
